...
function validateAgentConfig(config) {
  ...
  if (!agentId) errors.push('agentId is missing — register the bridge in the portal (Device agent → Add agent)');
...
/**
 * The portal's "Download setup" button produces a PowerShell (or shell)
...
function parseInstallerScript(text) {
  ...
  if (!agentId && !agentSecret) {
    return { ok: false, reason: 'no agentId/agentSecret found — paste the file the portal generated with "Download setup"' };
  }
