 ...
 *   * `--from-installer <file|->`  the script the portal's "Download installer"
...
async function acquireIdentity({ ctx, logger, options, prompter }) {
  ...
  if (options.fromInstaller) {
    ...
    logger.raw('----------------------');
    logger.raw('The portal generates the agent id and secret: ISAPI Bridge & Windows Agent →');
    logger.raw('Register agent → Download installer. Point this wizard at that file to avoid');
