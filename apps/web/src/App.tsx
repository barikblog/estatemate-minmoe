...
function People({ user }: { user: User }) {
  ...
  const available=useAsync<{ items:Row[] }>(()=>user.role==='admin'?api('/api/properties/available'):Promise.resolve({ items:[] }),[user.role]);
  const imports=useAsync<ListResponse<Row>>(()=>user.role==='admin'?api('/api/imports?kind=users&limit=20'):Promise.resolve({ items:[],page:1,limit:20 }),[user.role]);
  const [showForm,setShowForm]=useState(false);const [showImport,setShowImport]=useState(false);const [editing,setEditing]=useState<Row|null>(null);const [message,setMessage]=useState('');const [temporaryPassword,setTemporaryPassword]=useState('');
  ...
  return <PagePanel title="People" subtitle="Register, assign, import and safely manage resident and staff accounts" action={user.role==='admin'?<div className="row-actions"><button className="secondary" onClick={()=>setShowImport(!showImport)}>Import users</button><button className="primary" onClick={()=>{setShowForm(!showForm);setEditing(null);}}>Add person</button></div>:null}>
    ...
    {showForm&&<UserAccountForm properties={available.data?.items ?? []} onDone={(notice)=>{setShowForm(false);setMessage(notice);refresh();}} />}
    {editing&&<UserAccountForm user={editing} properties={available.data?.items ?? []} onDone={(notice)=>{setEditing(null);setMessage(notice);refresh();}} onCancel={()=>setEditing(null)} />}
    {showImport&&<UsersCsvImporter onDone={()=>{refresh();}} />}
    <div className="people-filters"><label>Search<input value={search} onChange={(event)=>setSearch(event.target.value)} placeholder="Name, email or phone" /></label><label>Role<select value={role} onChange={(event)=>setRole(event.target.value)}><option value="">All roles</option><option value="resident">Residents</option><option value="security">Security</option><option value="cashier">Cashiers</option><option value="admin">Administrators</option></select></label></div>
    ...
    {user.role==='admin'&&imports.data&&imports.data.items.length>0&&<section className="import-history"><h3>User import history</h3><DataTable rows={imports.data.items} columns={[['filename','File'],['status','Status'],['total_rows','Rows'],['successful_rows','Created'],['error_rows','Errors'],['created_at','Uploaded','date']]} action={(row)=>row.storage_key?<a className="text" href={`/api/files/${encodeURIComponent(String(row.storage_key))}`}>Download source</a>:null} /></section>}
  </PagePanel>;
}

function UserAccountForm({ user,properties,onDone,onCancel }: { user?:Row;properties:Row[];onDone:(message:string)=>void;onCancel?:()=>void }) {
  ...
  return <FormCard title={user?`Edit ${String(user.name)}`:'New account'} onSubmit={submit} message={message}>
    ...
    <label>Role<select name="role" value={role} onChange={(event)=>setRole(event.target.value)}><option value="resident">Resident</option><option value="security">Security</option><option value="cashier">Cashier</option><option value="admin">Administrator</option></select></label>
