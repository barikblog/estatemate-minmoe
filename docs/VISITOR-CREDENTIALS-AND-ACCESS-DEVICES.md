...
Residents never choose a gate. Their passes are stored with `gate_scope='both'`, meaning every gate, entry and exit, and the gate picker is not shown to them. Every currently usable every-gate pass is queued for every enabled access-control device, including devices that require manual synchronization; an agent-linked device is delivered automatically. Only an Administrator or Manager may attach a pass to one saved device, which stores `gate_scope='gate'` and queues it only for that device. The hourly reconciliation also catches passes created before a device was linked, and administrators/managers can immediately repeat that reconciliation with `POST /api/visitors/sync-active`. A visitor who overstays can still be checked out; an ended window blocks check-in only.
